...
def run_experiments():
    ...
    for query_type in ['Q1', 'Q2']:
        ...
        # Create plots for this query (COMMENTED OUT - ONLY PER-GROUP PLOTS)
        # COMMENTED OUT: Standard 4-panel comparison plots
        # plt.figure(figsize=(15, 10))
        # ... (standard plots commented out) ...
        ...
        # COMMENTED OUT: Standard 4-panel plots
        # Only running per-group experiments now
        pass        
        ...
        if greedy_valid:
            ...
            plt.plot(greedy_groups, greedy_vals, "b-", label=f"Greedy ({len(greedy_valid)} groups)", alpha=0.7, linewidth=1)
        
        if ilp_valid:
            ilp_groups, ilp_vals = zip(*ilp_valid)
            plt.plot(ilp_groups, ilp_vals, "orange", linestyle="-", label=f"ILP ({len(ilp_valid)} groups)", alpha=0.7, linewidth=1)
        
        if sketchrefine_valid:
            sketchrefine_groups, sketchrefine_vals = zip(*sketchrefine_valid)
            plt.plot(sketchrefine_groups, sketchrefine_vals, "g-", label=f"SketchRefine ({len(sketchrefine_valid)} groups)", alpha=0.7, linewidth=1)
...
def run_per_group_experiment(query_type):
    ...
    for group_idx, (group_id, group_data) in enumerate(group_list):
        if group_idx % 10 == 0:  # Progress indicator
            print(f"  Processing group {group_idx + 1}/{len(group_list)}...")
