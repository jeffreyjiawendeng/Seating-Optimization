...
def phase1_meta_table_query(meta_tables_df, group_size, brightness_threshold, query_type):
    ...
    if query_type == 'Q1':
        ...
        picked, counts = solve_ilp(
            sub=data_matrix,
            size_=group_size,  # We want to select enough tables to accommodate the group
            rep=group_size,     # Each table can be selected up to group_size times
    ...
    if picked:
        # Extract selected table IDs
        selected_table_ids = [int(data_matrix[i][3]) for i in picked]  # Table_ID is at index 3
